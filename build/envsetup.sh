...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop ro.omega.device | grep -q "$OMEGA_BUILD"); then
...
function installboot()
{
    ...
    if (adb shell getprop ro.omega.device | grep -q "$OMEGA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.omega.device | grep -q "$OMEGA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
