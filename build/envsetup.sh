...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop ro.lineage.device | grep -q "$OMEGA_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $OMEGA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop ro.lineage.device | grep -q "$OMEGA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.lineage.device | grep -q "$OMEGA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
